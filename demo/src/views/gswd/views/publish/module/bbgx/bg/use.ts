...
export default function (props: PublishProps) {
  ...
  const {formData, code} = useCommit(),
    ...
    content = computed(() => {
      const {bbh} = props,
        judge = data.value.filter((it: any) => it.zt != -1 && it.zt != 9).length,
        {fbdq} = formData.value,
        fwObj = (fbdqObj as any)[fbdq] || {};
      return [
        judge
          ? [
              ...
              getMarkDownCode([code.value[2].code, 'git push origin ' + bbh].join('\n'), 'bash'),
              getMarkDownTitle('归版（发版时合并至主程序）', 2),
              getMarkDownCode(['git checkout ' + fwObj.qh, 'git merge ' + bbh].join('\n'), 'bash')
            ].join('\n')
          : [
              getMarkDownTitle('终版（完全发版时合并至主程序）', 2),
              getMarkDownCode(['git checkout ' + fwObj.qh, 'git merge ' + bbh, 'git branch -d ' + bbh, 'git push origin -d ' + bbh].join('\n'), 'bash')
            ].join('\n'),
  ...
  function search() {
    magicPost('/242/magic/BB01/s-bbfbjl', {id: props.id}).then(({data: {list}}) => {
      ...
      data.value = list.map((it: any) => {
        const fwObj = (fbdqObj as any)[it.fbdq] || {};
