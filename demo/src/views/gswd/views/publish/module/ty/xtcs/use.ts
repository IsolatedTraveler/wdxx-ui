...
export function use(talbe: string = 'P_XTCS', primary: string[] = ['MKDM', 'XH']) {
  const formData = ref<{
    ...
    fwq: 'cs242',
    MKDM: '',
    bb: format(new Date(), 'v1.0.yyyyMMdd.01'),
    XH: '',
    ZYLX: '',
    CSM: '',
    CSSM: '',
  ...
  function query() {
    var { bb, MKDM, XH, fwq } = formData.value, arr = bb.split('.'), bb = '', tj = `MKDM = '${MKDM}'`, backTable = ''
    if (XH) {
      tj += ` and XH in ('${(XH + '').split(',').join("','")}')`
    }
    if (tj) {
      ...
      Promise.all([
        getTableCol(talbe, fwq),
        getCodes(tj, talbe, fwq)
      ]).then(([col, data]) => {
        if (!XH) {
          if (data && data.length) {
            formData.value.max = Math.max(...data.map(it => it.XH)) + 1
          } else
            formData.value.max = '0'
        } else
