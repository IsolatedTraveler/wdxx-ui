...
export function useHisTyDzfp(table: string) {
  ...
  function getDzpjJkpz(bb: string) {
    const { fbbb, fbjg, zsdz, fwq } = formData.value
      , backTable = bb ? `${table}_${bb}` : table, tj = `dm = '${fbbb}'`
    return Promise.all([
      getTableCol(table, fwq),
      getCodes(tj, table, fwq)
    ]).then(([col, data]) => {
      data.forEach(it => {
        console.log(it)
  ...
  function getDzpjJkcs(bb: string) {
    const table = 'T_DZPJ_JKCS', { fbbb, fbjg, fwq } = formData.value
      , backTable = bb ? `${table}_${bb}` : table, tj = `jkdm = '${fbbb}'`
    return Promise.all([
      getTableCol(table, fwq),
      getCodes(tj, table, fwq)
    ]).then(([col, data]) => {
