...
export function usePrody() {
  const formData = ref({fwq: '242', bb: useGswdStore().getBb, where: `mkbh='080901' and ywdm = 1`}),
    code = ref(['']),
    talbe = 'prody',
    primary = ['MKBH', 'YWDM'];
  function getCode() {
    ...
    if (tj) {
      ...
      Promise.all([getTableCol(talbe, obj.fwq), getPrody(tj, obj.fwq)]).then(([col, {data, ccgc}]) => {
        col = col.filter(({col}) => col != 'URL' && col != 'ROW_ID' && col != 'SFTB' && col != 'XTBH');
        data.forEach((it: any) => {
          it.SQL = (it.SQL || '').trim();
        });
        const {i, b, d} = dealSqlData(data, col, talbe, tj, primary, bb);
        code.value = [['-- 新增存储过程调用方案', b, d, i, 'commit;'].filter(it => it).join('\n'), ccgc];
      });
    } else {
      code.value = ['条件不能为空'];
    }
