...
export const componentIndex = (key: string, group: Array<string> = []) => {
  ...
  return [
    ...
    ...arrI,
    `export const ${firstMax(PKG_PREFIX)}${getName(key.split('-'))} = withInstall(${getName(key.split('-'))}${group.length ? `, {${group.map(it => getName(it.split('-'))).join(', ')}}` : ''
    })`,
    ...group.map(it => {
...
export const ${name1}Props = propsBuildS({
})
